...
async function startIsland(page, id) {
  ...
  await page.waitForFunction(() => window.CS.scenes.currentName === 'island' || document.querySelector('.contract-card') || document.querySelector('.panel-wishes-intro'), null, { timeout: 20000 });
  if (await page.$('.panel-wishes-intro:not(.panel-contract)')) { const r = await page.evaluate(() => { const p = document.querySelector('.panel-wishes-intro'); const before = p.scrollTop; p.scrollTop = p.scrollHeight; const b = [...p.querySelectorAll('button')].find((x) => x.textContent.includes('C’est parti')); const rb = b.getBoundingClientRect(); return { scrollable: p.scrollHeight > p.clientHeight, moved: p.scrollTop > before || p.scrollHeight <= p.clientHeight, visible: rb.bottom <= innerHeight + 1 && rb.top >= 0 }; }); if (r.scrollable) check(r.moved && r.visible, `île ${id} : la préparation défile jusqu’au bouton (visible=${r.visible})`); }
