...
(async () => {
  ...
  const m1 = await page.evaluate(() => window.CS.scenes.current.renderer.houle.f); await page.waitForTimeout(1200); const m2 = await page.evaluate(() => window.CS.scenes.current.renderer.houle.f);
  check(m1 > m2 && m2 > 0, `la houle se rapproche de la côte avec le temps (${m1.toFixed(2)} → ${m2.toFixed(2)})`);
