...
// posent, un voilier s'en va. Un toucher presse le pas, un second passe au bilan.
//
...
import { cadreCarte, habillerCarte } from './postcard.js';
...
export class Finale {
  ...
  finish() {
    ...
    this.r.transition = null; this.r.transitionSpeed = 1; this.r.finale = false; this.r.nu = false; this.r.nuit = 0;
  ...
  entrer(p) {
    ...
    else if (p === 'titre') { this.camB = this.cadre; this.bandes = 0; this.lettres = 0; this.r.nuit = 0; }
  }
  ...
  /**
   * Un jour et une nuit pendant le recul. Le soir tombe en un tiers du temps, la nuit tient un cinquième,
   * puis le jour revient — le paysage est ainsi rendu au plein jour avant qu'on le visite, ce qui est bien
   * la moindre des choses pour une île qu'on vient de finir en plein midi.
   */
  cycleJour(u) {
    this.r.nuit = u < 0.34 ? doux(u / 0.34) : u < 0.56 ? 1 : 1 - doux(clamp((u - 0.56) / 0.4, 0, 1));
    if (!this.hibou && u > 0.33) { this.hibou = true; this.sc.playSfx('fauna_owl', 0.3); }
    ...
    if (!this.aube && u > 0.92) { this.aube = true; this.sc.playSfx('fauna_arrive', 0.3); }
  }
