 ...
 */
export function transition(board, season) {
  const ev = [];
  ...
  if (season === 'spring') {
    ...
    for (const t of tiles) {
      if (t.dry) { t.dry = false; ev.push({ type: 'green', q: t.q, r: t.r }); }
      if (Board.isFamily(t, 'orchard')) {
        const hamlets = neighbors(t.q, t.r).map(([a, b]) => board.get(a, b)).filter((n) => n && Board.isFamily(n, 'hamlet'));
        for (const h of hamlets) ev.push({ type: 'harvest', q: t.q, r: t.r, to: { q: h.q, r: h.r }, pts: P.autumnHarvest });
      }
    }
