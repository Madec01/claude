...
export function buildTempoPrep({ onStart, onBack }) {
  ...
  append(root,
    ...
    h('div', { class: 'tp-regles' },
      regle('icon_target', `${T.cadran} secondes par tuile`, `Le temps se lit sur l’île : la lueur sur la mer se resserre vers la côte, et bat la mesure. À zéro, la tuile est perdue et sa case restera vide : −${T.vide} par case à la fin, −${T.vide + T.videBloque} si elle seule empêchait une région de fermer, −${T.videRegion} par case quand les vides se touchent.`),
