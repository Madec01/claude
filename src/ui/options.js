    ...
    h('div', { class: 'opt-cols' },
      h('div', { class: 'opt-col' },
        ...
        h('div', { class: 'opt-group' },
          ...
          (() => { const sel = h('select', { class: 'opt-select', 'aria-label': 'Relevé de saison' }, ...[['auto', 'Automatique'], ['full', 'Complet'], ['brief', 'Bref'], ['none', 'Aucun']].map(([v, l]) => h('option', { value: v }, l))); sel.value = Save.options.recap || 'auto'; sel.addEventListener('change', () => { Save.options.recap = sel.value; Save.save(); AudioSys.play('ui_click', { volume: 0.4 }); }); return h('div', { class: 'opt-row opt-btnrow' }, h('span', { class: 'opt-label' }, 'Relevé de saison', h('small', {}, 'Au changement de saison : complet (les gains ligne à ligne, un toucher le replie), bref (le total seulement), aucun. Automatique : complet sur ordinateur, bref sur téléphone.')), sel); })(),
