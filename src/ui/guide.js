...
const TABS = {
  ...
  hand: { label: 'File, main et semis', build: () => h('div', {},
    ...
    h('p', { class: 'g-intro' }, `Les prochaines tuiles sont visibles dans la file (trois, puis quatre ou cinq avec le Regard et la Longue-vue de l’Atelier). Avant l’île ${mechIsland('hand')}, la première est obligatoire : un souffle permet de l’échanger avec l’une des deux suivantes ou de la défausser.`),
    h('h3', {}, icon('icon_info'), `La main de saison (dès l’île ${mechIsland('hand')})`),
    h('p', { class: 'g-intro' }, `Dès l’île ${mechIsland('hand')}, la file devient une main : on joue n’importe quelle tuile visible en cliquant dessus (ou touches 2 à 5), sans souffle. L’échange disparaît ; défausse, poche et bourgeon restent.`),
  ...
  seasons: { label: 'Saisons', build: () => h('div', {},
    ...
    h('p', { class: 'g-note' }, `Dès l’île ${mechIsland('weather')}, la règle de chaque saison est tirée parmi ces trois variantes à chaque fois que la saison arrive (les îles d’avant gardent les règles de base). La règle en cours est affichée dans la boîte de saison et rappelée dans le journal.`),
    h('h3', {}, icon('icon_cloud'), 'Météo'),
    h('p', { class: 'g-intro' }, `Dès l’île ${mechIsland('weather')}, un événement peut être annoncé au début d’une saison ; il se déclenche à la mi-saison et dure jusqu’à la suivante.`),
    ...
    h('p', { class: 'g-note' }, `À chaque changement de saison, chaque animal présent rapporte ${B.points.faunaSeason} points, et un souffle pour ${B.breaths.faunaPer} animaux, et chaque sentier reliant deux villages rapporte ${B.points.pathSeason} point. Un sentier se trace tout seul entre deux hameaux séparés d’au plus trois tuiles de terre ouverte (prairie, champ, verger, lande, colline).`),
  ) },
  ...
  breath: { label: 'Souffles et vœux', build: () => h('div', {},
    ...
    h('p', { class: 'g-intro' }, `Les souffles sont une réserve de pouvoirs. On en gagne ${B.breaths.close} par région fermée, ${B.breaths.wish} par vœu exaucé, un pour ${B.breaths.faunaPer} animaux présents à chaque changement de saison et un par série de trois bons coups. Ils sont rares : chaque pouvoir est un renoncement.`),
    ...
    h('h3', {}, icon('icon_flag'), 'Le contrat d’archipel (dès le chapitre 2)'),
