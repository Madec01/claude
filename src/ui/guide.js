...
const TABS = {
  ...
  recipes: { label: 'Cahier', build: () => { const known = new Set(Save.data.campaign.recipes || []); return h('div', {},
    h('p', { class: 'g-intro' }, `Dès l’île ${mechIsland('fuse')}, toucher une tuile posée qui a une voisine avec laquelle elle fait recette propose la fusion (${B.fusion.cost} souffles, +${B.fusion.bonus} point, l’ordre des deux familles est indifférent, aucune tuile de la file n’y passe). La tuile touchée devient la tuile composée, la voisine reste ; la composée compte pour ses deux familles et rapporte à chaque saison. Une tuile déjà bâtie ne fusionne plus. La première fois qu’une recette est réalisée, elle s’écrit ici.`),
  ...
  breath: { label: 'Souffles et vœux', build: () => h('div', {},
    ...
    h('p', { class: 'g-intro' }, `Touche une tuile d’une région close (${B.build.cost} souffles, aucune tuile de la file n’y passe) : elle passe au niveau 2, tous ses bords valent +1 de plus, elle compte double dans la taille de sa région et son décor s’épaissit. Une tuile à la fois ; le niveau 3 se bâtit de même (${B.build.cost3} souffles) une saison plus tard. Les tuiles qui attendent une action portent un liseré doré sur l’île, et le bouton Bâtir du bandeau les compte ; toucher une tuile ouvre ses actions, avec leur coût et ce qu’elles rapportent.`),
    ...
    h('h3', {}, icon('icon_tree'), `La croissance (chapitre ${GROWTH_CHAPTER} seulement)`),
