...
| 18 bis | **Bâtir, fusionner, réparer : des actions sur une tuile posée** (refonte du 26 septembre 2026, retour du commanditaire) | Plus de tuile consommée. On touche une tuile posée : ses actions s'affichent avec coût et gain. Bâtir demande une région close (2 souffles, 3 au niveau 3, Charpente −1) ; fusionner se fait entre deux voisines qui font recette (2 souffles, la touchée devient la composée, la voisine reste) ; réparer une friche coûte 1. Rien ne compte comme une pose ; retour de tuile retiré ; liseré doré, bouton « Bâtir » qui compte, message quand rien n'est possible. Étoiles recalibrées. | **fait** |
| 18 ter | **Harmonie : trois fleurs** (26 septembre 2026, idée du commanditaire) | Variété, Équilibre, Achèvement, comptées à la fin de l'île (surface ÷ 5 par fleur), dès l'île 6 ; seuils calés pour que le robot fort, qui ne les vise pas, en obtienne une sur deux. Visibles sous le score, dans la tournée finale, sur la carte postale et au bilan. Étoiles recalibrées. | **fait** |
...
| Temps par tuile | trois, cinq ou huit secondes, au choix avant l'île (journal 151) ; retenu d'une partie à l'autre, un meilleur score par temps |
| Tutoriel | pas à pas, la première fois ou à la demande ; le temps s'arrête sous une carte (journal 154) |
...
| Tirage de saison | **fait** (journal 155) : dix bonus et dix malus, une carte par passage de saison ; 50 / 50 au départ, ±8 % par coup jugé (meilleur ou pire tiers des places), bornes 20 et 80 % ; Marée basse et Brume épaisse deux fois plus rares ; jamais la même carte deux fois de suite. À décider en jouant : garder les jalons ; les **effets de saison** (printemps éclaircies, été brume qui dérive, automne récolte, hiver gel) sont proposés, pas codés |
| Jamais collées | deux cases cachées ne se touchent jamais (journal 152) : chaque banc est une case seule, entourée de cases où poser ; mesuré : 24 à 29 % des cases, part déductible 0,35 à 0,46 (mieux qu'en bancs : plus d'indices par case), génération en moins d'une demi-seconde |
