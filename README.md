...
8. Dès l'île 6, les **vœux** des habitants donnent des objectifs à échéance : les exaucer rapporte des points, des souffles et une tuile rare (moulin, chapelle, tour de guet, puits, camp, puis dès l'île 13 le grenier, la ruche et le menhir, qui rapportent à chaque saison selon leurs voisines). Un vœu est toujours **faisable** : il promet ses tuiles à la file (« trois vergers collés à un hameau » promet quatre vergers, un de marge, et un hameau), qui les donne avant 80 % de l'échéance, réparties dans le temps, quel que soit le semis ; la file ne remplace le hasard que lorsqu'il est en retard sur ce calendrier (2,5 % des tirages de la campagne).
11. Dès l'île 11, on peut **bâtir** : toucher une tuile d'une **région close** (2 souffles, aucune tuile de la file n'y passe) la fait passer au niveau 2, ses bords valent +1 de plus, elle compte double dans sa région et son décor s'épaissit. Une tuile à la fois. Les tuiles qui attendent une action portent un liseré doré, le bouton Bâtir du bandeau les compte, et toucher une tuile ouvre ses actions avec leur coût et leur gain ; toucher une tuile sans action dit pourquoi (région ouverte, souffles qui manquent). Dès l'île 19, une tuile de niveau 2 qui a traversé une saison se bâtit une seconde fois (3 souffles) : niveau 3, bords +2, triple dans sa région, +1 par saison, et un nom selon sa famille (forêt ancienne, pâturage, domaine, bourg, grand verger, eau profonde, tourbière, pic, dune, alpage, grande lande), sans autre effet.
12. Dès l'île 13, on peut **fusionner** deux tuiles voisines qui font recette (2 souffles, aucune tuile de la file n'y passe) : champ + eau = rizière, hameau + champ = ferme, hameau + roche = fortin, roche + eau = cascade, forêt + roche = grotte, sable + eau = lagune. La tuile touchée devient la tuile composée, la voisine reste ; la composée compte pour ses deux familles et rapporte à chaque saison ; une tuile déjà bâtie ne fusionne plus. La première réalisation d'une recette s'écrit dans le Cahier du Guide.
...
15. Les **souffles** sont des pouvoirs limités, gagnés en fermant des régions (un) et en exauçant des vœux (deux) ; ils sont rares, chaque pouvoir est un renoncement : défausser la tuile (un), annuler la dernière pose (trois, une fois par saison), bâtir (deux, trois au niveau 3), fusionner (deux), réparer une friche (un). Bâtir, fusionner et réparer ne prennent rien dans la file et ne comptent pas comme une pose.
...
26. Dès l'île 6, trois **fleurs d'harmonie** sous le score récompensent une île variée et bien finie : **Variété** (cinq familles tiennent chacune une région d'au moins quatre tuiles, moins si la file de l'île en offre peu), **Équilibre** (aucune région ne couvre plus d'un cinquième des tuiles posées : le contrepoids des grandes régions) et **Achèvement** (au moins 85 % des tuiles dans des régions closes, et aucune friche). Chaque fleur ouverte à la fin de l'île rapporte la surface de l'île divisée par cinq ; une fleur qui s'ouvre ou se referme le dit, et les toucher explique ce qui manque. Elles s'ouvrent une à une avant les étoiles dans la tournée finale, sur la carte postale et au bilan, qui dit aussi ce qui a manqué. Seuils mesurés sur le robot fort, qui ne les vise pas : il obtient chacune environ une fois sur deux.
25. Les **grandes régions** paient davantage : à partir de cinq tuiles d'une même famille, chaque tuile qui agrandit la région gagne +1 (« grande région », +2 à partir de dix, « très grande région »), et la prime de fermeture est multipliée par 1,5 (par 2 à partir de dix), en plus du double des hameaux. Une carte l'explique la première fois.
18. Une pose qui **coûte des points** (bords et contraintes, total négatif) laisse une **friche** : ruine pour un hameau, lit asséché pour l'eau, terre morte ailleurs. Elle ne rapporte plus rien, ne compte plus pour sa famille (régions, vœux, faune) et ses bords ne valent rien pour les voisines. La toucher la remet en état (1 souffle, sans tuile) : seuls ses bons voisins comptent alors, et elle peut fermer une région.
...
| Bâtir (dès l'île 11), fusionner (dès l'île 13), réparer une friche | clic ou toucher sur une tuile posée à liseré doré : ses actions s'affichent en bas avec leur coût et leur gain ; un clic sur l'action (ou un second sur la tuile) la fait |
...
- **Le Souffle court** (grande tuile du menu, après l'île 6) : le mode nerveux, à part. Pas de file : la tuile arrive et trois secondes s'écoulent — le temps se lit juste au-dessus de l'île, sur un chronomètre : le gros chiffre (dixièmes en petit) qui respire sur la musique et tremble sur la dernière seconde, et dessous la marée, une barre d'eau qui se retire par les deux bouts, écume aux extrémités ; rouge sur la dernière seconde ; à zéro la tuile est perdue (le chiffre éclate, la barre flashe) et sa case restera vide (malus à la fin, plus fort si la case bloque une région ou si les vides se touchent). Poser dans le premier tiers du temps (une seconde à trois secondes par tuile, 2,7 s à huit) enchaîne une série qui multiplie les points de la tuile (×1,5 à 3, ×2 à 6, ×3 à 10), deux fois plus vite quand la place est bonne ; après chaque pose, un mot sur la tuile dit pourquoi la série monte, tient ou casse, et c'est le seul commentaire du mode (un total qui vole, pas de ruban). Une tuile perdue dit sur le moment ce qu'elle coûtera. Les saisons comptent double et changent le jeu : l'hiver gèle le cadran (×1,4), le printemps propose deux tuiles, l'été donne une réserve à répartir sur la saison (douze secondes à trois secondes par tuile, vingt à cinq, trente-deux à huit ; vide, ce qui restait à poser est perdu d'un coup, en une seule annonce), l'automne couvre l'île de brume que seule la pose dissipe. Le mode ne propose que les familles déjà rencontrées dans la campagne (pas de colline avant l'île 8, pas de lande avant la 10). Avant l'île, les règles en cinq lignes, un bouton « S'entraîner » (une petite île fixe : six poses guidées sans chrono, la case qui brille, une région qui se ferme, puis douze poses à huit secondes, sans record) et le choix du temps par tuile, huit secondes la toute première fois (« découverte »), puis trois, cinq ou huit secondes (« défi », « normal », « découverte ») (retenu d'une partie à l'autre, un meilleur score et une meilleure série par temps, jamais ceux d'un autre temps à leur place) ; sur l'île, un décompte (trois, deux, un, pose !) avant que le cadran ne parte, aussi au retour de pause — un seul à la fois, et la reprise repart exactement où le temps s'était arrêté. Avec le tutoriel, le temps s'arrête sous les cartes et la pose attend qu'on ait lu : c'est un entraînement, qui ne fait pas de record ; chaque saison a sa carte, montrée à sa première arrivée, et la bannière de saison dit d'abord l'effet du mode (réserve d'été, brume, cadran gelé, deux tuiles). La tuile à poser est seule, centrée juste sous l'île, de taille modeste (on regarde l'île) — jamais la suivante ; au printemps, les deux tuiles proposées sont côte à côte. Musique à part, composée avec Suno par le studio : trois pistes tirées au sort à chaque partie (« Woodblock Swing », 143 BPM ; « Woodblock Chase » et sa seconde prise, 152 BPM), qui partent sans fondu ; le décompte et le chronomètre se calent sur le tempo de la piste tirée, un pas tous les deux temps. Île procédurale à chaque partie (plus haute que large au téléphone en portrait, pour remplir l'écran), sans vœu ni souffle ; au bilan, son numéro et « Rejouer cette île » (même graine, même délai, même forme, pour comparer deux plans). Au doigt, une option du mode permet de poser d'un seul toucher, sans confirmation. Un **défi du jour** (bouton de l'écran de préparation) : la même île pour tout le monde, tirée de la date, cinq secondes par tuile, forme fixe, toutes les familles ; un record par jour, à part des autres. Un **objectif personnel** au choix avant la partie (au plus trois tuiles perdues, six poses rapides de suite, trois saisons sans perte), suivi sous les points et jugé au bilan, sans récompense. Réglages dans `BALANCE.tempo`.
- **Île infinie** : l'île grandit à chaque pose, sans fin ; le score et le nombre de saisons sont enregistrés.
- **Sous la brume** (s'ouvre avec l'Île infinie) : un tiers des cases de l'île cachent des tuiles déjà là ; on voit l'inventaire de ce qu'elles cachent, jamais où. On pose contre une tuile ou contre la brume (ce qui s'y cache est déjà une tuile), donc presque toute l'île est ouverte dès le départ. Une tuile posée contre la brume dit combien de ses voisines cachées sont de sa famille ; au passage de saison (cinq poses), une case assez entourée se dévoile et ses bords valent double. Un jalon par saison (annoncer une famille : juste +5 et bords triples, faux −5), un crayon pour noter, un déplacement qui coûte la prochaine tuile, un trésor caché (+8), −3 par case restée cachée à la fin. Deux crans : Brume claire (inventaire exact) et Brume épaisse (inventaire par couleur, un indice sur deux, jalon obligatoire). Chaque île est passée à un solveur qui dose la part déductible, dans un worker pour ne pas geler le téléphone. Sans étoiles ni graines ; meilleur score par cran. Deux cases cachées ne se touchent jamais : chaque banc de brume est entouré de cases où l'on pose, là où l'indice se lit. On choisit sa case sans voir les points, et la main montre les cinq tuiles de la saison. À chaque passage de saison, un tirage : une carte, bonus ou malus, pour la saison qui commence (dix de chaque : Longue-vue, Lanterne, Deux jalons, Marée basse, Bords dorés, Boussole, Déplacement offert, Crayon sûr, Prime de dévoilement, Main large ; La brume gagne, Indices muets, Main courte, Jalon forcé, Brume épaisse, Mauvais voisinage, Nuit noire, Tuile perdue, Crayon effacé, Vent contraire). Première saison à 50 / 50 ; chaque bon coup (meilleur tiers des places possibles) pousse la chance vers le bonus de 8 %, chaque mauvais coup vers le malus, entre 20 et 80 %. Le tirage tourne à l'écran comme une roue, puis la carte reste sous le bandeau de saison avec la chance à côté. Au menu, les trois modes à part (Souffle court, Sous la brume, Île infinie) ont des tuiles de même taille, sous le grand bouton de l'histoire qui s'ouvre en deux (continuer, choisir une île). Les deux modes ont leur tutoriel pas à pas, la première fois ou à la demande (case à cocher avant l'île) ; au Souffle court, le temps s'arrête tant qu'une carte est affichée.
...
- **Atelier des saisons** (bouton du menu, qui s'allume quand une amélioration est à portée ; le bilan d'une île offre un bouton « Atelier · N à portée » quand les graines paient quelque chose, et l'Atelier s'ouvre de lui-même sur le chemin de l'île suivante la première fois qu'une amélioration devient payable — il note ce qu'il a montré, et ne revient que pour du nouveau) : des améliorations permanentes achetées en graines, ouvertes chapitre par chapitre (regard et patience au chapitre 1 ; souffle de départ et semence rare au 2 ; source, refuge et almanach au 3 ; charpente au 4 ; alambic et manteau au 5 ; maître d'œuvre au 7 ; étoile du soir ensuite, douze en tout). Les suivantes restent visibles, grisées, avec leur chapitre.
...
Deux vitesses : `tools/suite.sh court` (~1 min 30, les cinq tests Node et `gate.js` en fumée) avant chaque
...
node tests/brume.test.js          # Sous la brume : génération, indices, solveur, dévoilement, jalons, déplacement, fin, reprise
node tests/brume.js               # Sous la brume au navigateur (iPhone 12) : menu, cran, fiche d'une case, Déplacer, bilan ; SHOTS=dossier pour des captures
...
node tools/calibrate.js 4 1-30 --write   # calibrage des étoiles : bot fort (tests/bot.js) et glouton, 4 graines par île, écrit src/data/campaign_stars.js (seuils 45/65/85 % de la médiane du bot fort jusqu'à l'île 10, 40/58/75 % ensuite, l'or à 100 %)
